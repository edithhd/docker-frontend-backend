# Workshop Workbook — [EDITH DOROTHY]
...
**Jawaban:**
Tidak ada, karena masih menggunakan versi mock yang belum ada back end nya sehingga requestnya juga belum ada.
...
```
[User click] → [Store di-state "step: scoring" di function attachVerifikasiDokumen file VerifikasiDokumen.js] → [Memanggil function scheduleScoring di App.js] → [Render hasil scoring di file HasilScoring.js] → [UI render hasil]
```

### Quiz Sesi 1

1. Browser -> Frontend -> Back End/API -> Database
2. Karena mock API tidak sesuai dengan API aslinya, tetapi bisa dimitigasi debgab API contract
3. Kalau scoreWithMock menggunakan data mock / data dummy, sedangkan callUatBackend memanggil API dengan method fetch dari JavaScript dan menggunakan data real.
